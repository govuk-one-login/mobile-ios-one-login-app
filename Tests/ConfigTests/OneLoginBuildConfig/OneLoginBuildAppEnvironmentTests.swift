...
struct OneLoginBuildAppEnvironmentTests {
    let sut = AppEnvironment.self
    
    @Test
    func test_plistValues() {
        #expect(Bundle.main.bundleIdentifier == "uk.gov.onelogin.build")
        #expect(Bundle.main.infoDictionary?["CFBundleDisplayName"] as? String == "One Login - Build")
    ...
    func test_appEnvironment_helpers() {
        // Helpers
        #expect(sut.buildConfiguration == "Build")
