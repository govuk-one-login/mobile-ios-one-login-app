...
extension WalletNetworkClientWrapperTests {
    ...
    func test_authRequestSucceeded() async throws {
        ...
        let response = try await sut.request(URLRequest(url: URL(string: "testurl.com")!)).withAuthentication(scope: "test wallet scope").execute()
    ...
    func test_authRequestError() async throws {
        ...
        mockSessionManager.sessionState = .expired
        do {
            _ = try await sut.request(URLRequest(url: URL(string: "testurl.com")!)).withAuthentication(scope: "").execute()
        } catch let error as SessionError {
            XCTAssert(error == SessionError.expired)
        }
        await fulfillment(of: [exp], timeout: 5)
